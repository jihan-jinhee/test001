...
* 코드에 대한 간략한 설명
   ...
   * 위에서 작성한 Bank 인터페이스를 사용한 BankImpl(POJO: Plane Old Java Object)를 구현한다. 여기에는 순수한 데이터만 들어가며 비지니스 로직은 포함되지 않는다.(모델과 로직의 분리)
...
* 스프링은 비지니스 논리를 POJO로 구현하여, 테스트, 보수/개선이 개념적으로 더 쉽고 간단하다.
   * POJO는 순수하게 도메인에 초점을 맞춘다. 엔터프라이즈 프렘워크에 의존하지 않는다.
